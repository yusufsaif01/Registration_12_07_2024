...
module.exports = (router) => {
  ...
  router.get("/member/public/profile/:user_id/:sent_by", function (req, res) {
    try {
      ...
      responseHandler(
        req,
        res,
        serviceInst.getPublicProfileDetails(
          req.params.user_id,
          req.params.sent_by
        )
      );
  ...
  router.get(
    "/member/player/list",
    checkAuthToken,
    userValidator.playerListQueryValidation,
    function (req, res) {
      ...
      sortOptions = {
        ...
        sort_order:
          req.query && req.query.sort_order ? req.query.sort_order : 1,
      };
      filter = {
      ...
      filterConditions = {
        ...
        profile_status:
          req.query && req.query.profile_status
            ? req.query.profile_status
            : null,
        email_verified:
          req.query && req.query.email_verified
            ? req.query.email_verified
            : null,
      };
      let serviceInst = new UserService();
      responseHandler(
  ...
  router.get(
    "/member/club/list",
    checkAuthToken,
    userValidator.clubAcademyListQueryValidation,
    function (req, res) {
      let paginationOptions = {};
      let sortOptions = {};
      let filter = {};
   ...
   */
  router.put(
    "/member/status-activate/:user_id",
    checkAuthToken,
    function (req, res) {
      try {
   ...
   */
  router.put(
    "/member/status-deactivate/:user_id",
    checkAuthToken,
    function (req, res) {
      try {
        if (!req.params.user_id) {
  ...
  router.get(
    "/member/search",
    checkAuthToken,
    userValidator.memberSearchQueryValidation,
    function (req, res) {
      let paginationOptions = {
        page_no: req.query && req.query.page_no ? req.query.page_no : 1,
        limit:
      ...
      responseHandler(
        req,
        res,
        serviceInst.getMemberList({ filter, paginationOptions })
      );
    }
  );
};
